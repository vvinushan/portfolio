...
OpeningDoor::OpeningDoor(b2World * world, sf::Vector2f position, float angle)
{
	m_fMotionTime = 1.5f;
	m_state = CLOSED;
	...
	l_shape.SetAsBox(0.7f * 0.5f, 0.04f * 0.5f, b2Vec2(-0.85f, -0.10f), 90.f);
	m_leftSide->CreateFixture(&l_fixtureDef);
	l_shape.SetAsBox(0.7f * 0.5f, 0.04f * 0.5f, b2Vec2(-0.85f, 0.10f), 0.f);
	m_leftSide->CreateFixture(&l_fixtureDef);
	l_shape.SetAsBox(0.18f * 0.5f, 0.18f * 0.5f, b2Vec2(-1.11f, 0.f), 90.f);
	m_leftSide->CreateFixture(&l_fixtureDef);
...
void OpeningDoor::update(float timestep)
{
	if (m_state == OPENING)
	{
...
void OpeningDoor::draw(sf::RenderTarget & target, sf::RenderStates states) const
{
	target.draw(m_door1);
	target.draw(m_door2);
	target.draw(m_side1);
